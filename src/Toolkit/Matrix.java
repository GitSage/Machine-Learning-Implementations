...
public class Matrix {
    ...
    ArrayList<Integer> rowsToCopy;


    public final static double MISSING = Double.MAX_VALUE; // representation of missing values in the dataset
    ...
    public Matrix (Matrix that, int rowStart, int colStart, int rowCount, int colCount, ArrayList<Integer> myToCopy) {
        if(myToCopy == null){
            myToCopy = new ArrayList<>();
        }
        rowsToCopy = new ArrayList<>();
        m_data = new ArrayList< double[] >();
        if (colStart == -1) {
            for (int i = 0; i < that.rows(); i++) {
                if (myToCopy.contains(i)) {
            ...
            for(int i = 0; i < that.rows(); i++) {
                if (that.get(i, rowStart) == colStart) {
                    double[] rowSrc = that.row(i);
                    double[] rowDest = new double[colCount];
                    for(int j = 0; j < colCount; j++)
                        rowDest[j] = rowSrc[j];
    ...
    /**
     * Almost identical to mostCommonValue(int), but only accepts if the classifcation is the same.
     * @param col
     * @param classification
     * @return
     */
    public double mostCommonValue(int col, double classification) {
    ...
    public ArrayList<Integer> getToCopy() {
        return rowsToCopy;
    }

	/**
	 * Finds the most common value in the chosen column that exists in the given set of instances.
	 * @param col the column (attribute) to be checked.
	 * @param instances the allowable instances.
     * @return
     */
	public double mostCommonValue(int col, Set<Integer> instances) {
		...
		TreeMap<Double, Integer> tm = new TreeMap<>();
		for(int i = 0; i < rows(); i++) {
			if (instances.contains(i)) { // only include the value if it's a specified instance.
				double v = get(i, col);
				if(v != MISSING) {
					Integer count = tm.get(v);
					if(count == null)
